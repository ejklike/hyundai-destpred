...
def train_eval_save(car_id, proportion, dest_term, model_id, data_num='all', n_save_viz=0):
    """
    TRAIN and EVAL for given car and experimental settings
    """
    # Load datasets
    fname_trn = os.path.join(
        DATA_DIR.format(data_num=data_num),
...
def main(_):
    ...
    data_num_list = ['all', 200, 400, 600, 800, 1000]
    preprocess_methods = ['proportion', 'minute']
    if FLAGS.preprocess:
        for raw_data_fname in RAW_DATA_FNAME_LIST:
            for data_num, preprocess_method in product(data_num_list, preprocess_methods):
                data_preprocessor = DataPreprocessor(data_num, to_dir=DATA_DIR.format(data_num=data_num))
                data_preprocessor.process_and_save(raw_data_fname, method=preprocess_method)
