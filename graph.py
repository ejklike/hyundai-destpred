...
def embed_path(paths, model_type=None, path_embedding_dim=None):
    # print('path embedded')
    model_type = FLAGS.model_type if model_type is None else model_type
    path_embedding_dim = FLAGS.path_embedding_dim if path_embedding_dim is None else path_embedding_dim
    if model_type == 'dnn':
...
def extract_feature(path_t, meta_t, keep_prob=None, reg_scale=None, n_hidden_layer=None, n_hidden_node=None,
                    use_meta=None, use_path=None, model_type=None, path_embedding_dim=None):
    """ return final prediction results
    """
    # Regularization parameters
    keep_prob = FLAGS.keep_prob if keep_prob is None else keep_prob
    reg_scale = FLAGS.reg_scale if reg_scale is None else reg_scale
    # dense layer parameters
    n_hidden_layer = FLAGS.n_hidden_layer if n_hidden_layer is None else n_hidden_layer
    n_hidden_node = FLAGS.n_hidden_node if n_hidden_node is None else n_hidden_node

    # Embed path and meta data
    embedding_results = []

    use_meta = FLAGS.use_meta if use_meta is None else use_meta
    use_path = FLAGS.use_path if use_path is None else use_path
    if use_meta:
        meta_embedding = embed_meta(meta_t)
        embedding_results.append(meta_embedding)

    if use_path:
        path_embedding = embed_path(path_t, model_type=model_type, path_embedding_dim=path_embedding_dim)
