...
RECORD_FNAME = 'result_dest5_global_dxdy.csv'
...
def train_eval_save(car_id_list, dest_term, model_id, n_save_viz=0):
  ...
  path_trn, meta_trn, dest_trn, dt_trn, full_path_trn = \
      ...
      train=True, seq_len=FLAGS.seq_len, data_dir=DATA_DIR)
  path_tst, meta_tst, dest_tst, dt_tst, full_path_tst = \
      unified_latest_seqdata(car_id_list, proportion_list, dest_term,
      train=False, seq_len=FLAGS.seq_len, data_dir=DATA_DIR)
  ...
  # FOR TARGETING CARS
  for car_id in [5, 100, 29, 72, 50, 14, 9, 74]:
    # LOAD DATA
    path_trn, meta_trn, dest_trn, dt_trn, full_path_trn = \
    ...
    if FLAGS.record:
      ...
      recorder.append_values(['car{:03}'.format(car_id),
                              model_id, 
                              len(path_trn), 
                              len(path_tst), 
                              global_step,
...
def main(_):
  ...
  for i, params in enumerate(param_product):
    ...
    train_eval_save(car_id_list, args.dest_type, model_id, n_save_viz=FLAGS.n_save_viz)
