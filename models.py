...
def model_fn(features, labels, mode, params):
    ...
    if params['use_cluster']:
        dest_centroids = features['dest_centroid']
        n_centroids = dest_centroids.get_shape().as_list()[1]
        x = tf.layers.dense(x, n_centroids, activation=tf.nn.relu)
        x = tf.expand_dims(tf.nn.softmax(x), axis=1)
        predictions = tf.squeeze(tf.matmul(x, dest_centroids))
    else:
