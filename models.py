...
def embed_car_id(car_ids, params):
    car_id_W = _variable_on_cpu('car_id_table', [params['n_car'], 7])
    car_id_embedding = tf.nn.embedding_lookup(car_id_W, car_ids, name='car_id_embedding')
    return car_id_embedding
...
def build_graph(features, params):
    ...
    # CAR_ID embedding
    car_ids = features['car_id']
    car_ids = tf.cast(car_ids, dtype=tf.int32)
    car_id_embedding = embed_car_id(car_ids, params)
    concat_target.append(car_id_embedding)
    ...
    if params['cluster_bw'] > 0:
        ...
        predictions = tf.layers.dense(x, 2, activation=None, name='final')
