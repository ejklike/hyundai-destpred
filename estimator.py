...
def train_eval_save(car_id_list, proportion, dest_term,
    ...
    try:
        ...
        fname_tst_list = [os.path.join(DATA_DIR, get_pkl_file_name(car_id, proportion, dest_term, train=False))
                          for car_id in car_id_list]
        ...
        for car_id, fname_tst in zip(car_id_list, fname_tst_list):
            ...
            # input functions for evaluation each car_id
...
def main(_):
    ...
    short_term_dest_list = [0] if FLAGS.dest_type is None else [FLAGS.dest_type]
    proportion_list = [0.0, 0.2, 0.4, 0.6, 0.8] if FLAGS.proportion is None else [FLAGS.proportion]
    ...
    n_hidden_layer_list = [FLAGS.n_dense] if FLAGS.n_dense is not None else [1]
    cluster_bw_list = [FLAGS.cband] if FLAGS.cband is not None else [0, 0.01, 0.1, 0.3]

    # PARAM GRIDS
    param_grid_targets = [short_term_dest_list,  # destination
    ...
    for i, params in enumerate(param_product):
        dest_term, proportion, use_meta, path_embedding_dim, k = params[:5]
        n_hidden_layer, cluster_bw = params[5:]
        ...
        id_components = ['dest{:02}'.format(dest_term if dest_term > 0 else 0),
        ...
        train_eval_save(car_id_list, proportion, dest_term,
...
if __name__ == "__main__":
    ...
    print(FLAGS.proportion, FLAGS.dest_type, FLAGS.cband, FLAGS.path_dim, FLAGS.n_dense)
    print(unparsed)
