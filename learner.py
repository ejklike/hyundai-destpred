...
class Model(object):
    ...
    def prepare_prediction(self, dest_trn, cband=None):
        """
        ...
        """
        cband = FLAGS.cband if cband == None else cband
        # Find centroids of destinations
        if cband > 0:
            self.clustering = ModifiedMeanShift(radius=1.2, bandwidth=cband,
            ...
            print('cband=%f, #cluster=%d' % (cband, len(self.cluster_centers_)),
    ...
    def build_graph(self, learning_rate=None, k=None, max_length=None, model_type=None, gpu_mem_frac=None,
        ...
        # Placeholders
        path_shape = dict(
            dnn=[None, 2 * 2 * k],  # fixed size
            rnn=[None, max_length, 2]  # variable length
        )[model_type]
        ...
        feature_t = extract_feature(self.path_t, self.meta_t, keep_prob=keep_prob, reg_scale=reg_scale,
                                    n_hidden_layer=n_hidden_layer, n_hidden_node=n_hidden_node, use_meta=use_meta,
                                    use_path=use_path, model_type=model_type, path_embedding_dim=path_embedding_dim)
